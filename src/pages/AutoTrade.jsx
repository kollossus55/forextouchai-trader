        ...
        runningBots.forEach(async (bot) => {
            ...
            if (Math.random() > 0.5) { // 50% chance per tick
                ...
                try {
                    ...
                    const technicalSignal = technicalAnalysisService.analyzeSignal(historicalData, bot.strategy_type);

                    // Candlestick Pattern Analysis
                    const candlestickPattern = candlestickPatternService.detectPatterns(historicalData);

                    // Chart Pattern Analysis
                    const chartPattern = chartPatternService.detectPatterns(historicalData);
                    ...
                    if (technicalSignal && technicalSignal.confidence >= (bot.min_confidence || 75)) {
                        finalSignal = technicalSignal.signal;
                        finalConfidence = technicalSignal.confidence;
                        analysisSource = 'Technical Indicators';
                    }

                    // Boost confidence if patterns confirm
                    if (candlestickPattern && candlestickPattern.signal === finalSignal) {
                        finalConfidence = Math.min(99, finalConfidence + 5);
                        analysisSource += ` + ${candlestickPattern.pattern}`;
                        addLog(bot.name, `📊 ${candlestickPattern.description}`, 'info');
                    }

                    if (chartPattern && chartPattern.signal === finalSignal) {
                        finalConfidence = Math.min(99, finalConfidence + 5);
                        analysisSource += ` + ${chartPattern.pattern}`;
                        addLog(bot.name, `📈 ${chartPattern.description}`, 'info');
                    }

                    // Generate signal if confidence threshold met
                    if (finalSignal && finalConfidence >= (bot.min_confidence || 75)) {
                        ...
                        if (isCrypto) pipMultiplier = 10.0;

                        const sl = finalSignal === 'BUY' 
                            ? realPrice - (bot.stop_loss_pips * pipMultiplier) 
                            : realPrice + (bot.stop_loss_pips * pipMultiplier);

                        const tp = finalSignal === 'BUY' 
