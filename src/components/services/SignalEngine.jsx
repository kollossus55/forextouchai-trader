...
function insufficientData(timeframe, count) {
    return {
        signal: 'INSUFFICIENT_DATA',
        confidence: 0,
        available: false,
        reason: `Only ${count} real ${timeframe} candles available. ` +
                `Live indicator analysis needs 60+. Attach the EA to this symbol so it uploads ` +
                `broker candles. Signals generated by your bots use broker candles server-side ` +
...
export function computeSignal(symbol, timeframe, currentPrice) {
    ...
    if (!candles || candles.length < 60) {
        return insufficientData(timeframe, candles ? candles.length : 0);
    }
    return computeFromCandles(symbol, timeframe, candles);
}

// Compute a signal from pre-built candles (real broker OHLC fetched via the
// getMarketCandles backend function). This is the path the Pairs grid uses so
// it shows true prices and real indicators instead of the in-memory tick store,
// which can never accumulate 60 H4 candles in a single browser session.
...
function computeFromCandles(symbol, timeframe, candles) {
    const closes = candles.map(c => c.close);
    ...
    // UNKNOWN D1 bias means we don't have daily candles — treat it as no
    // constraint, not a conflict. The previous logic capped EVERY directional
    // signal at 74 when D1 was UNKNOWN, so no Top Pick could ever clear 75.
    const d1Conflicts = d1.bias !== 'NEUTRAL' && d1.bias !== 'UNKNOWN' && signalBias !== 'NEUTRAL' && d1.bias !== signalBias;
    const d1LiveConflicts = d1.bias !== 'NEUTRAL' && d1.bias !== 'UNKNOWN' && liveSignalBias !== 'NEUTRAL' && d1.bias !== liveSignalBias;
