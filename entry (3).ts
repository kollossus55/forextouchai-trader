    ...
    try {
        ...
        if (Array.isArray(body.candles) && body.candles.length) {
            ingestCandles(base44, body.candles)
                .catch(e => console.warn('[BRIDGE] candle ingest failed:', e.message));
        }
...
async function ingestCandles(base44, payload) {
    for (const entry of payload.slice(0, 12)) {
        ...
        if (!symbol || !timeframe || rawBars.length < 50) continue;
        ...
        if (bars.length < 50) {
            console.warn(`[BRIDGE] ${symbol} ${timeframe}: only ${bars.length}/${rawBars.length} bars survived validation`);
            continue;
        }
        ...
        try {
            ...
            console.warn(`[BRIDGE] CandleHistory write failed for ${symbol} ${timeframe}:`, e.message);
        }
    }
}
