...
input double MaxSpreadPips     = 3.0;   // FOREX ONLY. Ignored for indices, metals and crypto - a "pip" means nothing there.
input double MaxSpreadPercent  = 0.05;  // Universal spread ceiling, as % of price. Normal is 0.007-0.014%, so 0.05% blocks blowouts without blocking indices. 0 = no check.
...
input int    CandleUploadSecs  = 60;     // Seconds between candle uploads. One symbol+timeframe per upload, rotating.
...
void ExecuteSignalObj(string obj) {
   ...
   double pipSize    = PipSizeFor(symbol, digits, point);
   double spreadPrice = spreadPts * point;
   double spreadPips  = (pipSize > 0) ? spreadPrice / pipSize : 0;
   double spreadPct   = (price > 0) ? (spreadPrice / price) * 100.0 : 0;

   // A "pip" is a forex convention and does NOT translate across asset classes.
   // Judging an index by MaxSpreadPips blocked every one of them: NAS100 with a
   // 2.0-point spread scores 20 "pips" against a limit of 3, even though as a
   // fraction of price that spread (0.010%) is TIGHTER than EURUSD's (0.014%).
   //
   // So the universal gate is percentage of price, which is comparable across
   // forex, indices, metals and crypto. MaxSpreadPips is kept as an additional
   // check for forex only, where the unit is familiar and meaningful.
   bool isForexPair = IsForexSymbol(symbol);

   if(MaxSpreadPercent > 0 && spreadPct > MaxSpreadPercent) {
      Print("[BRIDGE MT5] REJECTED: Spread ", DoubleToString(spreadPrice, digits),
            " = ", DoubleToString(spreadPct, 4), "% of price > MaxSpreadPercent ",
            DoubleToString(MaxSpreadPercent, 4), "% for ", symbol);
      ReportExecutionFailure(id, symbol, -1);
      return;
   }
   if(isForexPair && MaxSpreadPips > 0 && spreadPips > MaxSpreadPips) {
      Print("[BRIDGE MT5] REJECTED: Spread ", DoubleToString(spreadPips, 1),
            " pips > MaxSpreadPips ", MaxSpreadPips, " for ", symbol);
      ReportExecutionFailure(id, symbol, -1);
      return;
   }
   ...
   double requestedLot = (sigLot > 0) ? sigLot : FixedLotSize;
   double lot = requestedLot;
   if(volStep > 0) lot = MathFloor(lot / volStep) * volStep;

   // Do NOT raise the lot to the broker minimum. The app sizes each position
   // from risk % and the real stop distance; quietly bumping 0.03 up to a
   // 0.10 minimum trades more than three times the intended risk, and nothing
   // anywhere would have said so.
   //
   // Minimum volume is per-SYMBOL, so the app cannot check it — indices often
   // have a far higher minimum than forex. This is the only place it can be
   // caught.
   if(lot < minVol) {
      Print("[BRIDGE MT5] REJECTED: ", symbol, " needs ", DoubleToString(requestedLot, 4),
            " lots for the intended risk, but the broker minimum is ", DoubleToString(minVol, 4),
            ". Taking the minimum would trade ", DoubleToString(minVol / MathMax(requestedLot, 0.0001), 1),
            "x the intended risk. Either raise risk_per_trade_percent, increase the account balance, ",
            "or remove ", symbol, " from this bot.");
      ReportExecutionFailure(id, symbol, -2);
      return;
   }
...
// Rotating cursor over (symbol x timeframe).
int candleCursor = 0;
...
string BuildCandlePayload() {
   ...
   // ONE symbol+timeframe per heartbeat, rotating. Sending every combination at
   // once meant a half-megabyte POST and dozens of server-side writes, which
   // exceeded MT5's 20-second WebRequest limit (error 1003/5203). This was
   // fixed in v1.16 and lost again in v1.17 — restored here.
...
//+------------------------------------------------------------------+
//| v1.18: instrument classification                                 |
//+------------------------------------------------------------------+
// Mirrors the pip sizes in the app's _shared/instruments.ts. Both sides must
// agree on what a pip is for an instrument, or the app's stop distances and the
// EA's spread reporting describe different things.

bool IsForexSymbol(string brokerSymbol) {
   string b = BareSymbol(brokerSymbol);
   if(StringLen(b) != 6) return false;
   // Six letters, both halves a known currency code
   string ccy = "USDEURGBPJPYCHFCADAUDNZDSEKNOKDKKSGDHKDMXNZARTRYPLNCZKHUFCNH";
   string base  = StringSubstr(b, 0, 3);
   string quote = StringSubstr(b, 3, 3);
   return (StringFind(ccy, base) >= 0 && StringFind(ccy, quote) >= 0);
}

bool IsIndexSymbol(string brokerSymbol) {
   string b = BareSymbol(brokerSymbol);
   string idx = "|US500|SPX500|SP500|US30|DJI|NAS100|US100|USTEC|UK100|GER40|DE40|GER30|DE30|FRA40|ESP35|JPN225|AUS200|HK50|EU50|SWI20|";
   return StringFind(idx, "|" + b + "|") >= 0;
}

bool IsMetalSymbol(string brokerSymbol) {
   string b = BareSymbol(brokerSymbol);
   return (b == "XAUUSD" || b == "XAGUSD" || b == "GOLD" || b == "SILVER");
}

// Pip size per instrument, matching the app's table.
double PipSizeFor(string brokerSymbol, int digits, double point) {
   string b = BareSymbol(brokerSymbol);

   if(b == "XAUUSD" || b == "GOLD")   return 0.1;
   if(b == "XAGUSD" || b == "SILVER") return 0.01;

   if(IsIndexSymbol(b)) {
      if(b == "US30" || b == "DJI" || b == "JPN225" || b == "HK50") return 1.0;
      if(b == "UK100" || b == "GER40" || b == "DE40" || b == "GER30" ||
         b == "DE30"  || b == "FRA40" || b == "AUS200" || b == "EU50") return 0.5;
      if(b == "ESP35") return 1.0;
      return 0.1;   // US500, NAS100 and friends
   }

   if(IsForexSymbol(b))
      return (StringSubstr(b, 3, 3) == "JPY") ? 0.01 : 0.0001;

   // Unknown (crypto, energies, exotics): fall back to the broker's own
   // convention rather than inventing a number.
